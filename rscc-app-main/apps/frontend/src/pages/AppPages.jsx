...
const Badge = ({ children, tone = "lime" }) => <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${tone === "slate" ? "bg-slate-100 text-slate-600" : tone === "red" ? "bg-red-50 text-red-700" : "bg-lime-100 text-lime-800"}`}>{children}</span>;
...
export function Landing() { return <div className="min-h-screen bg-[#101b2a] text-white"><header className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8"><Link to="/" className="flex items-center gap-2"><span className="grid h-9 w-9 place-items-center rounded-xl bg-lime-400 font-black text-slate-950">R</span><b>RSCC</b></Link><Link className="rounded-xl bg-lime-400 px-4 py-2 text-sm font-bold text-slate-950" to="/login">Academy portal</Link></header><main className="mx-auto grid max-w-7xl gap-10 px-5 pb-20 pt-14 sm:px-8 lg:grid-cols-2 lg:py-28"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-lime-300">Rising Star Cricket Club</p><h1 className="mt-4 text-5xl font-black leading-[.95] tracking-tight sm:text-7xl">Build your game.<br/><span className="text-lime-300">Find your edge.</span></h1><p className="mt-6 max-w-xl leading-7 text-slate-300">One connected academy space for coaches, players, teams, training attendance and fixtures.</p><div className="mt-8 flex gap-3"><Link className="rounded-xl bg-lime-400 px-5 py-3 font-bold text-slate-950" to="/register">Join RSCC <ArrowRight className="ml-1 inline" size={17}/></Link><Link className="rounded-xl border border-white/20 px-5 py-3 font-bold" to="/login">Sign in</Link></div></div><div className="rounded-3xl bg-gradient-to-br from-lime-400 to-emerald-400 p-8 text-slate-950"><p className="text-xs font-bold uppercase tracking-widest">Connected academy portal</p><p className="mt-24 text-4xl font-black">Train. Track.<br/>Perform.</p><p className="mt-5 max-w-sm text-sm font-medium">Manage your academy’s players, daily attendance, squads and fixtures from one place.</p></div></main></div>; }

export function Dashboard() { const { loading, error, data } = useLoad(async () => { const [players, teams, matches, user] = await Promise.all([playersService.list(), teamsService.list(), matchesService.list(), authService.me().catch(() => ({ data: { user: JSON.parse(localStorage.getItem("user") || "null") } }))]); return { players: players.data.players, teams: teams.data.teams, matches: matches.data.matches, user: user.data.user }; }); const cards = [["Players", data?.players?.length || 0, Users], ["Teams", data?.teams?.length || 0, Users], ["Matches", data?.matches?.length || 0, Trophy]]; return <Page title={`Welcome${data?.user?.name ? `, ${data.user.name}` : ""}.`} eyebrow="Academy overview"><div className="grid gap-4 sm:grid-cols-3">{cards.map(([label, value, Icon]) => <Card key={label}><Icon className="text-lime-700" size={20}/><p className="mt-5 text-3xl font-black">{loading ? "—" : value}</p><p className="text-sm text-slate-500">{label}</p></Card>)}</div>{error && <p className="mt-5 text-sm text-red-700">{error}</p>}<div className="mt-5 grid gap-5 lg:grid-cols-2"><Card><h2 className="font-black">Quick actions</h2><div className="mt-4 grid gap-2">{[["Register player", "/players"], ["Mark attendance", "/attendance"], ["Schedule match", "/matches"]].map(([label, to]) => <Link key={to} className="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3 text-sm font-bold hover:bg-lime-50" to={to}>{label}<Plus size={16}/></Link>)}</div></Card><Card><h2 className="font-black">Upcoming fixtures</h2><div className="mt-4 space-y-3">{data?.matches?.slice(0, 3).map(match => <MatchRow key={match._id} match={match}/>) || <p className="text-sm text-slate-500">Loading fixtures…</p>}</div></Card></div></Page>; }
...
function MatchRow({ match }) { return <Link to={`/matches/${match._id}`} className="block rounded-xl bg-slate-50 p-3 hover:bg-lime-50"><b className="block text-sm">{matchTitle(match)}</b><small className="text-slate-500">{formatDate(match.details?.matchDate)} · {match.details?.ground || match.details?.venue || "Venue TBC"}</small></Link>; }
...
export function FutureModule({ type }) {
  const descriptions = {
    Tournaments: "Tournament creation, fixtures, standings and competition management are being prepared.",
    "Live Scoring": "Ball-by-ball match scoring, live scoreboards and match control are planned for a future release.",
    Statistics: "Player and team performance statistics will be available here in a future release.",
    "Offline Scoring": "Offline-first match scoring is planned for a future release.",
    "Advanced Analytics": "Advanced performance dashboards and analytics are planned for a future release.",
  };
  return <Page title={type} eyebrow="Advanced module">
    <Card className="overflow-hidden bg-[#101b2a] p-0 text-white">
      <div className="p-7 sm:p-10">
        <Badge>Coming soon</Badge>
        <h2 className="mt-5 text-3xl font-black sm:text-4xl">We're building {type.toLowerCase()}.</h2>
        <p className="mt-4 max-w-2xl leading-7 text-slate-300">{descriptions[type] || "This feature is currently under development."}</p>
        <div className="mt-7 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
          Your current RSCC modules remain fully available while this feature is being developed.
        </div>
      </div>
    </Card>
  </Page>;
...
export function Profile() {
  ...
  return <Page title="My profile" eyebrow="Account">
    <Card className="max-w-2xl">
      {loading ? "Loading profile…" : error ? <><p>{error}</p><Link className="mt-4 inline-block font-bold text-lime-700" to="/login">Sign in</Link></> :
      <div>
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          <div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-2xl bg-slate-900 text-2xl font-black text-lime-300">
      ...
      data.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.map(c =>
        <Card key={c._id}>
          <div className="flex items-start justify-between gap-3">
            <div><h2 className="font-black">{c.name}</h2><p className="mt-1 text-sm text-lime-700">{c.specialty}</p></div>
...
export function Fees() {
  ...
  return <Page title="Fees & payments" eyebrow="Academy billing">
    ...
    {showForm && <Card className="mb-5"><form onSubmit={submit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <button className="rounded-xl bg-lime-400 px-4 py-3 text-sm font-bold text-slate-950">Save fee</button>
...
export function Calendar() {
  ...
  return <Page title="Calendar" eyebrow="Academy schedule">
    ...
    {showForm && <Card className="mb-5"><form onSubmit={createEvent} className="grid gap-3 sm:grid-cols-2">
      ...
      <button className="rounded-xl bg-lime-400 px-4 py-3 text-sm font-bold text-slate-950">Add to calendar</button>
    </form>{message && <p className="mt-3 text-sm text-slate-600">{message}</p>}</Card>}
    {loading ? <Empty>Loading calendar…</Empty> : error ? <Empty>{error}</Empty> : data.length ? <div className="space-y-3">{data.map(i => <Card key={i.type+i.id}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><Badge tone={i.type === "Match" ? "lime" : "slate"}>{i.type}</Badge><h2 className="mt-3 font-black">{i.title}</h2><p className="mt-1 text-sm text-slate-500">{formatDate(i.date)}</p>{i.location && <p className="mt-1 text-sm text-slate-500">{i.location}</p>}</div><CalendarDays className="text-lime-700" size={22}/></div>
