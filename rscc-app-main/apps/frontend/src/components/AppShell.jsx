        ...
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-7">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Link aria-label="Notifications" to="/notifications" className="relative rounded-xl border border-slate-200 bg-white p-2.5 hover:border-rscc-blue">
              ...
              {unreadCount > 0 && <span className="absolute -right-1.5 -top-1.5 grid min-h-5 min-w-5 place-items-center rounded-full bg-rscc-red px-1 text-[10px] font-extrabold text-white">{unreadCount > 99 ? "99+" : unreadCount}</span>}
            </Link>
            <Link to="/profile" className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-1.5 pr-3 hover:border-rscc-blue">
              <span className="grid h-8 w-8 place-items-center overflow-hidden rounded-lg bg-blue-50 text-xs font-extrabold text-rscc-blue">
                {(() => { try { const stored = JSON.parse(localStorage.getItem("user") || "{}"); return stored.avatar ? <img src={stored.avatar} alt="" className="h-full w-full object-cover"/> : user.initials; } catch { return user.initials; } })()}
              </span>
