...
const notify = async (title, body, type) => {
  const users = await User.find({ status: "active" }).select("_id notificationPreferences");
  const key = type === "event" ? "events" : "announcements";
  const notifications = users.filter(user => user.notificationPreferences?.[key] !== false).map(user => ({ user: user._id, title, body, type }));
...
export const createEvent = async (req, res) => {
  try { const event = await Event.create({ ...req.body, createdBy: req.user._id }); await notify("New event: " + event.title, event.description || event.place, "event"); res.status(201).json({ success: true, event }); }
